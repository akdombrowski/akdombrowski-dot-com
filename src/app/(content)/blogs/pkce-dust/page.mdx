...
@property {typeof import('@/components/videos/VideoContainer.tsx').VideoContainer} VideoContainer
 */}
  <VideoContainer video={{ "title": "Visualizing the OAuth Flow and Why PKCE is Needed", "url": "https://www.youtube.com/embed/_zWovo2zv6k" }} />
...
2. A code challenge (or, equivalently, the transformed code_verifier,
      t(code_verifier))
   that was created BEFORE the request is sent
3. A transformation method
      (the t() function)
   Then, when the client gets back the message, instead of simply sending the authorization code to the token
   ...
   code_verifier, it can&apos;t obtain the access token—and therefore the resource is
   protected. The authorization server will deny requests for an access token
   without the correct secret.

{/**
- @typedef MDXProvidedComponents
- @property {typeof import('@/components/mdx/BlogImg.tsx').BlogImg} BlogImg */}
  <BlogImg
    src="/PKCE-attack-graphic-blog.webp"
    containerStyles={{ aspectRatio: 700 / 314, backgroundColor: "#FFF" }}
...
[*]:
    [https://pingidentity.com/en/resources/blog/post/pkce-dust-securing-your-oauth-2-0-authorization-code-grant-type.html](https://pingidentity.com/en/resources/blog/post/pkce-dust-securing-your-oauth-2-0-authorization-code-grant-type.html)
